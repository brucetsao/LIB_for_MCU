 ...
 *
 * \return The character or -1 if a faliure occures.  A failure is indicated
 * by the stream state.
 */
int istream::get() {
  int16_t c;
 ...
 * Extract a character if one is available.
 *
 * \param[out] c location to recieve the extracted character.
 *
 ...
 * Extract characters. 
 *
 * \param[out] str Location to recieve extracted characters.
 ...
 * \return always returns *this. A failure is indicated by the stream state.
 */
 //////////////////////////////////////////when is failbt set how is gcount handled?//////////////////////
 ...
 */
 void istream::getStr(char *str) {
  ...
  int16_t c;
  uint16_t i = 0;
  uint16_t m = width() ? width() - 1 :0XFFFF;
  if (i < m) {
    getpos(&pos);
    c = readSkip();
  }
